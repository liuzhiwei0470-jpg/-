...
router.post('/', validateBody(createSchema), async (req, res, next) => {
    try {
        ...
        // 自动同步一次文章
        try {
            await rssService.syncSubscription(subscription.id, subscription.routeUrl, subscription.filterInclude, subscription.filterExclude);
        }
        catch (error) {
...
router.put('/:id', validateBody(updateSchema), async (req, res, next) => {
    try {
        const subscription = await subscriptionService.update(parseInt(req.params.id), req.user.userId, req.body);
        if (!subscription) {
            return res.status(404).json({
                success: false,
                error: {
