...
export async function getRow(sql: string, ...args: any[]): Promise<any> {
  const client = getDb();
  const result = await client.execute({ sql, args });
  return result.rows[0] || null;
}

export async function getAllRows(sql: string, ...args: any[]): Promise<any[]> {
  const client = getDb();
  const result = await client.execute({ sql, args });
  return result.rows as any[];
}

export async function runSql(sql: string, ...args: any[]): Promise<RunResult> {
  const client = getDb();
  const result = await client.execute({ sql, args });
  return {
