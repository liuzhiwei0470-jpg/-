  ...
  <div class="article-list-page">
    <header class="page-header">
      ...
      <div class="header-right">
        ...
        <button @click="handleSync" class="btn btn-secondary" :disabled="syncing">
    ...
    <main class="page-main">
      ...
      <div v-else-if="articles.length === 0" class="empty-state">
        ...
        <h3>暂无文章</h3>
        <p>{{ emptyTip }}</p>
        <button @click="handleSync" class="btn btn-primary">刷新订阅</button>
      </div>
...
import { subscriptionApi, articleApi, type Subscription, type Article } from '@/api';
...
const subscriptionId = computed(() => parseInt(route.params.id as string));
...
function goToDetail(id: number) {
  router.push({
    path: `/subscriptions/${subscriptionId.value}/articles/${id}`,
    query: { from: 'subscription-articles', subscriptionId: subscriptionId.value.toString() },
  });
}
...
async function loadSubscription() {
  try {
    const res = await subscriptionApi.get(subscriptionId.value);
...
async function loadArticles(page: number = 1) {
  ...
  try {
    ...
    const res = await articleApi.list(subscriptionId.value, params);
...
async function loadUnreadCount() {
  try {
    ...
    unreadCount.value = res.data.bySubscription?.[subscriptionId.value] || 0;
  } catch (e) {
...
async function handleSync() {
  syncing.value = true;
  try {
    await articleApi.sync(subscriptionId.value);
    pagination.value.page = 1;
    await loadArticles();
    await loadUnreadCount();
  } catch (e) {
    toast.error('同步失败，请检查网络或订阅地址');
  } finally {
    syncing.value = false;
  }
}
...
async function handleMarkAllRead() {
  ...
  try {
    await articleApi.markAllAsRead(subscriptionId.value);
